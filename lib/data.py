...
#custom
from lib.dataset.casting_dataset import Castingdataset
...
def load_data(opt):
    ...
    if opt.dataroot == '':
        ...
        print(f'set dataRoot to {opt.dataroot}')


    if opt.dataset in ['casting_2']:
        print('casting')
        splits = ['train', 'test']
        ...
        dataset = {}
        dataset['train'] = Castingdataset(root= opt.dataroot ,train=True, transform= transform)
        dataset['test'] = Castingdataset(root= opt.dataroot,train=False, transform= transform)
        # dataset['train'].data, dataset['train'].targets, \
        # dataset['test'].data, dataset['test'].targets = get_casting_anomaly_dataset(
        #     trn_img=dataset['train'].data,
        #     trn_lbl=dataset['train'].targets,
        #     tst_img=dataset['test'].data,
        #     tst_lbl=dataset['test'].targets,
        #     abn_cls_idx = classes[opt.],
...
def get_casting_anomaly_dataset(trn_img, trn_lbl, tst_img, tst_lbl, abn_cls_idx=0, manualseed=-1):
    """[summary]
    Arguments:
        trn_img {np.array} -- Training images
        trn_lbl {np.array} -- Training labels
